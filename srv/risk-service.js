...
module.exports = cds.service.impl(async function () { 
    ...
    this.on("READ", BusinessPartners, async (req) => { 
        //Montamos el filtro para que no nos devuelva los que no tienen nombre
    ...
    this.on("READ", Risks, async (req, next) => { 
        ...
        the risk entity and the business partner entity are in different systems (SAP BTP and S/4 HANA Cloud), if there is such an expand, remove it */ 
        ...
        /* Instead of carrying out the expand, issue a separate request for each business partner This code could be optimized, 
        instead of having n requests for n business partners, just one bulk request could be created */
        try { 
            res = await next(); 
